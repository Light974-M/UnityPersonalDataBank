...
namespace UPDB.Physic.CustomTimeScale
{
    ...
    ///</summary>
    [AddComponentMenu(NamespaceID.UPDB + "/" + NamespaceID.Physic + "/" + NamespaceID.CustomTimeScale + "/StaticTimeController")]
        ...
        private void SaveState()
        {
            if (_timeScale > 0)
            {
                if (_timer >= (1 / _listDefilFrequency) / _timeScale)
                {
                    ...
                    {
                        //ne capture pas si immobile(donc d�cale par rapport aux autres objets
