...
[ExecuteAlways]
public class CircularMazeWallWidthGenerator : MonoBehaviour
{
    [SerializeField]
        ...
        for (int z = 0; z < _height; z++)
        {
            ...
            for (int x = 0; x < _length; x++)
            {
                ...
                for (int y = 0; y < _width; y++)
                {
                    ...
                    {
                        GameObject tile = Instantiate(_wallTilePrefab, new Vector3(x - wallKeyPointEraserX + (IsImpair(x) && IsPair(y) ? 0.5f : 0), z, y - wallKeyPointEraserY), Quaternion.identity);
                        tile.transform.SetParent(transform);
                        tile.transform.localPosition = new Vector3(0, z, (y - wallKeyPointEraserY) + 2);
